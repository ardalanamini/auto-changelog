...
export async function updateChangelogFile(
  ...
  if (sectionHeading.length === 0) sectionHeading = `## Release ${process.env.GITHUB_REF}`;

  let existingContent = "";
  if (await fs.promises.stat(changeLogFilePath)) {
    existingContent = (await fs.promises.readFile(changeLogFilePath)).toString();
  }

  const updatedContent = createNewContent(existingContent, changeLog, fileHeading, sectionHeading);

  info(`Writing new or updated changelog file`);
  await fs.promises.writeFile(changeLogFilePath, updatedContent);
}

function createNewContent(existingContent: string, newContent: string, fileHeading: string, sectionHeading: string): string {
  let updatedContent = "";
  if (existingContent.length === 0) {
    updatedContent = `${fileHeading}\r\r${addNewReleaseSection(newContent, sectionHeading)}`;
  } else {
